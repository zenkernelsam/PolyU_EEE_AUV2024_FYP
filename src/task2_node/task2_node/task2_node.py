...
class Task2Node(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(GetTask, 'task2', self.get_task_callback) #Create a Server (service) to listen Master Node's request
...
def main(args=None):
    ...
    task2_node = Task2Node()
    input = sys.argv[1:]
    task2_node.write_response(input[0],input[1],input[2]) 
    """
    You can the run "ros2 run task2_node task2_node False abc abc" which will response:
    ...
    Angle: 123 [uint32]
    """
    rclpy.spin(task2_node)
    task2_node.destroy_node()
    rclpy.shutdown()