...
class ControlNode(Node):
    def __init__(self):
        ...
        #Create some variables to clean prnding requests
        self.future_master_count = 0
        self.future_buoyancy_count = 0
        self.future_master_status = True
        self.future_buoyancy_status = True
        #Debug
        #print("Here0")
            ...
            if control_node.future_master.done():
                ...
                if control_node.buoyancy_count < 5:
                    ...
                    if response.buoyancy_direction != "Neutral":
                        ...
                        if control_node.future_buoyancy.done() == False:
                            control_node.future_buoyancy_count += 1
                            if control_node.future_buoyancy_count > 20:
                                control_node.get_logger().warn('[!] Requesting to Buoyancy Node --> [buoyancy_node] is timed out. The pending request is removed.')
                                control_node.client_to_master.remove_pending_request(control_node.future_buoyancy) #Find how to remove pending request in ROS2 rclpy API documentation.
                                control_node.future_buoyancy_count = 0
                            time.sleep(0.2)
