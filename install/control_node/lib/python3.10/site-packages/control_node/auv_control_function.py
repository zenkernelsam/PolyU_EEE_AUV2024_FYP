...
def thruster_move(thruster,period,d_cycle,t):
    ...
    if thruster == "left":
        ...
        call_bash("pwmchip2", period, duty_cycle_L)
        call_bash("pwmchip3", period, duty_cycle_R)
    elif thruster == "water_pump":
        call_bash("pwmchip4", period, duty_cycle_L)
    else:
...
def auv_init():
    ...
    #Initialize water_pump's PWM signal: 50Hz, 80% or others (then 12V*0.8=9.6V)
    thruster_move("water_pump",period, pump_voltage,0)
...
def thruster_system(parameter, time, angle):
    print(f"[O] Hardware Control: Thruster; Direction: {parameter}")
    #Using time control
